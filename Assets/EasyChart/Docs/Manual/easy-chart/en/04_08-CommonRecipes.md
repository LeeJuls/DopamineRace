...
## 1. Before you start: minimum checklist
...
## 2. Line chart (Line): Category X + numeric Y
...
## 3. Bar chart (Bar): centered categories + Y starts from 0
...
## 4. Grouped bars (Grouped Bar): multiple series share the same categories
...
## 5. Stacked bars (Stacked Bar): stacked + stackGroup
...
## 6. Scatter chart (Scatter): Value X/Y + hover + sizeMapping
...
## 7. Heatmap chart (Heatmap): (x, y, value) triplets
...
## 8. Radar chart (Radar): dimension index + value
...
## 9. Interaction/tooltip stability: SeriesData.id
