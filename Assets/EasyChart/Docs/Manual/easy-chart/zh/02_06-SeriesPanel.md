...
- **Header（标题行）**：折叠按钮 + 标题 + Help + Clone
- **Body（主体）**：Name / Id / Type / Settings / Data
- **Footer（右下角控制）**：↑ ↓ X

---

## Header：折叠/展开、Help、Clone
...
- Header 右侧的 **Clone** 图标按钮
- 点击会克隆当前 serie，在数组末尾插入一个副本
- 克隆时会自动生成新的唯一 `id` 和递增的 `name`（如 `Serie 1` → `Serie 2`）
- 用于快速创建相似配置的新系列
...
- 在 JSON Injection 面板中选择合适的 `Feed Mode`（Compact/Standard/Full）
- Copy 到外部编辑器批量生成/替换数据
- 粘贴回来 ApplyToChart

### 4) 快速派生相似系列

- 配置好一个 serie 后，点击 **Clone** 按钮
- 在新克隆的 serie 上微调数据或样式
